...
class Reviewer:
    ...
    async def review_job_v2(self, job: ReviewJob) -> None:
        ...
        try:
            ...
            # ----------------------------------------------------------------
            reviewer_result = next((r for r in results if r.role == ReviewRole.REVIEWER), None)
...
def _build_v2_comment(
    ...
    if reviewer_result and reviewer_result.decision:
        ...
        decision_icon = (
            "✅" if decision == "APPROVE" else "❌" if decision == "REQUEST_CHANGES" else "💬"
        )
    ...
    if reviewer_result:
        sections.append("---\n")
        sections.append(f"### 🎯 Final Review\n\n{reviewer_result.findings}\n")
