...
from src.config import AppConfig, GitLabConfig, ModelConfig, Provider, ReviewTarget, BranchRules, PromptsOverride
...
from src.reviewer import Reviewer, set_database, _severity_count
...
class TestReviewFilters:
    ...
    async def test_draft_mr_skipped(self, reviewer, mock_gitlab, mock_llm, db, cfg_with_target):
        mock_gitlab.get_mr.return_value = MRInfo(
            project_id=42, iid=7, title="Draft: wip", description="",
            author="bob", source_branch="f", target_branch="main",
            is_draft=True, web_url=""
        )
        set_database(db)
        with patch("src.reviewer._make_gitlab_client", return_value=mock_gitlab), \
...
class TestHappyPath:
    ...
    async def test_review_posted_to_gitlab(self, reviewer, mock_gitlab, mock_llm, db, cfg_with_target):
        set_database(db)
        with patch("src.reviewer._make_gitlab_client", return_value=mock_gitlab), \
             patch("src.reviewer._make_llm_client", return_value=mock_llm), \
             patch("src.reviewer.get_config", return_value=cfg_with_target):
    ...
    async def test_diff_hash_marked_seen(self, reviewer, mock_gitlab, mock_llm, db, queue, cfg_with_target):
        set_database(db)
        with patch("src.reviewer._make_gitlab_client", return_value=mock_gitlab), \
             patch("src.reviewer._make_llm_client", return_value=mock_llm), \
             patch("src.reviewer.get_config", return_value=cfg_with_target):
...
class TestAutoApprove:
    ...
    async def test_auto_approve_triggered_when_no_issues(
        self, reviewer, mock_gitlab, mock_llm, db
    ):
    ...
    async def test_auto_approve_blocked_on_critical(
        self, reviewer, mock_gitlab, mock_llm, db
    ):
    ...
    async def test_no_auto_approve_when_disabled(self, reviewer, mock_gitlab, mock_llm, db, cfg_with_target):
        set_database(db)
        with patch("src.reviewer._make_gitlab_client", return_value=mock_gitlab), \
             patch("src.reviewer._make_llm_client", return_value=mock_llm), \
             patch("src.reviewer.get_config", return_value=cfg_with_target):
