...
class TestGetConfig:
    ...
    async def test_webhook_secret_masked(self, app):
        ...
        assert webhook_secret != "test-secret" or webhook_secret == ""
    ...
    async def test_api_key_masked_in_providers(self, app):
        ...
        await app.post("/api/v1/providers", json={
            "id": "secret-provider",
            "name": "Secret",
            "type": "openai_compat",
            "url": "http://x",
...
class TestPutConfig:
    ...
    async def test_put_updates_queue_settings(self, app):
        r = await app.put("/api/v1/config", json={
            "queue": {"max_concurrent": 7, "max_queue_size": 200}
        })
        assert r.status_code == 200
    ...
    async def test_put_updates_model_temperature(self, app):
        await app.put("/api/v1/config", json={
