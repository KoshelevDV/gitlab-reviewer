...
from src.llm_client import LLMClient, get_model_info, list_models
...
class TestChat:
    ...
    async def test_chat_returns_content(self, client):
        respx.post(f"{BASE}/v1/chat/completions").mock(
            return_value=Response(
                200, json={"choices": [{"message": {"content": "Looks good to me."}}]}
            )
        )
    ...
    async def test_chat_fallback_to_native_ollama(self, client):
        ...
        respx.post(f"{BASE}/v1/chat/completions").mock(return_value=Response(404))
        respx.post(f"{BASE}/api/chat").mock(
            return_value=Response(200, json={"message": {"content": "Fallback response."}})
        )
...
class TestListModels:
    ...
    async def test_list_models_sends_api_key(self):
        route = respx.get(f"{BASE}/v1/models").mock(return_value=Response(200, json={"data": []}))
...
class TestGetModelInfo:
    ...
    async def test_get_model_info_error_returns_empty(self):
        respx.post(f"{BASE}/api/show").mock(return_value=Response(404))
