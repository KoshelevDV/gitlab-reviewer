...
from src.reviewer import parse_review_sections, _severity_count
...
class TestParseReviewSections:
    ...
    def test_case_insensitive_markers(self):
        text = (
            '<!-- review_inline file="x.py" line="5" -->\n'
            'Problem.\n'
            '<!-- review_endinline -->'
        )
        comments, _ = parse_review_sections(text)
    ...
    def test_empty_body_skipped(self):
        text = (
            '<!-- REVIEW_INLINE file="x.py" line="1" -->\n'
            '\n'
            '<!-- REVIEW_ENDINLINE -->'
        )
        comments, _ = parse_review_sections(text)
    ...
    def test_whitespace_in_markers_allowed(self):
        text = (
            '<!--  REVIEW_INLINE  file="a.py"  line="3"  -->\n'
            'Note.\n'
            '<!--  REVIEW_ENDINLINE  -->'
        )
        comments, _ = parse_review_sections(text)
...
class TestGetMrDiffRefs:
    ...
    async def test_returns_first_version_when_multiple(self):
        ...
        respx.get("http://gitlab/api/v4/projects/42/merge_requests/7/versions").mock(
            return_value=Response(200, json=[
                {"id": 10, "base_commit_sha": "latest", "start_commit_sha": "s", "head_commit_sha": "h"},
                {"id": 5, "base_commit_sha": "old", "start_commit_sha": "s2", "head_commit_sha": "h2"},
            ])
        )
        gl = GitLabClient("http://gitlab", "tok")
        try:
...
class TestLoadSeenFromDb:
    ...
    async def test_loads_hashes_from_db(self, db):
        ...
        await db.save_review(ReviewRecord(
            project_id="10", mr_iid=1, status="posted",
            diff_hash="abc123",
        ))
        await db.save_review(ReviewRecord(
            project_id="10", mr_iid=2, status="posted",
            diff_hash="def456",
        ))
...
class TestDbMigration:
    ...
    async def test_list_diff_hashes_excludes_empty(self, db):
        await db.save_review(ReviewRecord(
            project_id="1", mr_iid=1, status="posted", diff_hash="aa"
        ))
        await db.save_review(ReviewRecord(
            project_id="1", mr_iid=2, status="posted", diff_hash=""
        ))
        await db.save_review(ReviewRecord(
            project_id="1", mr_iid=3, status="error", diff_hash="bb"
        ))
...
class TestReviewerInlineFlow:
    ...
    async def test_no_inline_when_disabled(self, db, prompt_engine, queue):
        ...
        from src.gitlab_client import MRInfo, FileDiff
        ...
        mock_mr = MRInfo(
            project_id=1, iid=1, title="T", description="", author="a",
            source_branch="f", target_branch="main", is_draft=False, web_url=""
        )
        mock_diffs = [FileDiff(old_path="a.py", new_path="a.py",
                               diff="+x=1", new_file=False, deleted_file=False, renamed_file=False)]
        gl = AsyncMock()
    ...
    async def test_inline_comments_posted_when_present(self, db, prompt_engine, queue):
        """When LLM returns REVIEW_INLINE blocks and refs are available, post_mr_discussion is called."""
        from src.reviewer import Reviewer
        from src.gitlab_client import MRInfo, FileDiff
        from src.config import AppConfig, Provider, ModelConfig, GitLabConfig
        from src.queue_manager import ReviewJob
    ...
    async def test_inline_fallback_to_summary_when_no_refs(self, db, prompt_engine, queue):
        ...
        mock_diffs = [FileDiff(old_path="a.py", new_path="a.py", diff="+x",
